    ...
    >
      <template #content>
        <div class="flex flex-col h-[70vh]">
          <!-- Error State -->
          <div v-if="error" class="flex items-center justify-center py-12 text-red-500">
            <div class="text-center">
              <UIcon name="i-lucide-alert-circle" class="w-8 h-8 mx-auto mb-2" />
              <p>{{ error }}</p>
              <UButton size="sm" variant="outline" class="mt-2" @click="loadVoices">
                {{ $t("retry") }}
              </UButton>
            </div>
          </div>
          ...
          >
            <template #voices-leading="{ item }">
              <UAvatar
                :icon="playingVoices.has(item.id) ? 'lucide:pause' : 'lucide:play'"
                size="md"
                :ui="{
                  root: 'rounded-lg',
                }"
            ...
            <template #voices-trailing="{ item }">
              <div class="flex items-center gap-2">
                <UButton
                  :label="
                    tempSelectedVoice?.id === item.id ? $t('selected') : $t('select')
                  "
                  variant="ghost"
                  :color="tempSelectedVoice?.id === item.id ? 'primary' : 'neutral'"
                  @click.stop="onSelectVoice(item)"
                  :class="{
                    'flex': tempSelectedVoice?.id === item.id,
                    'sm:hidden group-hover:flex': tempSelectedVoice?.id !== item.id,
                  }"
                  :icon="tempSelectedVoice?.id === item.id ? 'lets-icons:check-fill' : 'ep:right'"
                />
