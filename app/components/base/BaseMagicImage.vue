<template>
  <div class="magic-image-container relative">
    ...
    <div v-if="loading" class="loading-overlay absolute inset-0 flex flex-col items-center justify-center bg-black/80 text-white">
      <span class="text-lg mb-2">{{ loadingText }}</span>
      <div class="progress-container w-48 h-2 bg-gray-700 rounded-full overflow-hidden">
        <div class="progress-bar h-full bg-primary" :style="{ width: `${loadingProgress}%` }"></div>
      </div>
    </div>
    <div v-if="showControls" class="controls absolute bottom-4 left-1/2 transform -translate-x-1/2">
      <UButton @click="triggerTransformation" :disabled="isTransforming">
